...
def blit_text(surface, text, rect, fontName, size, color):
    ...
    for word in words:
        ...
        if test_line_width + x - rect.x > rect.width:
    ...
    if line and y + line_spacing <= rect.bottom:
        line_surface = render_text(' '.join(line), color, size)
        text_rect = line_surface.get_rect(center=(x + rect.width/2, y))
        #surface.blit(line_surface, text_rect)
        blitObj(surface, line_surface, x + rect.width/2, y)
...
async def main():
    if screenWindow.useWeb:
        ...
        playSound(f'music/menuMusic.ogg', -1)
    global inGame, fullscreen, frame, validAlly, validEnemy, party, selectionSprite, selectedAlly, inSkinSelect, skinSelect, skinList, partyIndex, option
    ...
    global gameState, startBattle, opponent, opponentSelect, inOpponent, battleData, selectedAward, codeString, previewMode
    hud.cameraY=0
            ...
            if gameModeType == 0 or gameModeType == 2:
                ...
                if startBattle and not inCharSelect:
                    ...
                    if character.party != [] or battleData != None:
                        ...
                        for num, i in enumerate(validEnemy):
                            ...
                            character.opponentObj[i.name] = theAllies[num]
                ...
                for num in range(0, 5):
                    ...
                    if not inCharSelect or inSetting:
                        ...
                        if xButton.clicked:
                            if party[num] != None:
                                del screenWindow.animatedSprites[party[num].name + "__Character"]
                            party[num] = None
                ...
                for num, ally in enumerate(tempParty):
                    ...
                    charSprite = loadAnimatedSprite(ally.name + "__Character", ally.name, pos=(width+400, y), baseScale=0.4)
                ...
                for num, ally in enumerate(validAlly):
                    ...
                    if ally in party and not inSkinSelect:
                    ...
                    if not inCharSelect:
                        x = -100
                    if ally not in party or inSkinSelect:
                        allySprite = loadSprite(ally.name, f'selectionIcon/{ally.name}.png', baseScale=0.75, pos=(x, y), mouseCollide=True, layer=screenWindow.backGroundScreen)
                    ...
                    if inSkinSelect:
                        ...
                        if ally == party[partySelect]:
                            ...
                            blit_text(hud.surface, party[partySelect].info, pygame.Rect(60, 425, 560, 280), "gameFont.otf", 40, WHITE)
                ...
                mouseScroll = -1300 if mouseScroll < -1300 else 0 if mouseScroll > 0 else mouseScroll
