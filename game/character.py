...
class Skill:
    ...
    def __init__(self, name, cost=10, info='Skill', type='ally:', baseNum=10, num=10, numCap=30, usePercent=False, cooldown='0T', targetType='Single',
                 repeatHowMany=1, targetAmount=1, useAnim=False, animType='Thrower', stackDuration=False, stackAmplifier=False,
                 applyEffect=None, removeEffect=None, maxAmplifier=None, maxDuration=None, changePhaseTo=None, useDialogue=None, color=WHITE):
        if applyEffect == None:
            applyEffect = {}
...
class Act:
    def __init__(self, name, desc, sparingPoints, energyPoints, superPoints, checkAttack, useOn, color=WHITE):#, usePattern, number):
...
class Fight:
    def __init__(self, name, infoClass, classType, info, range='Default', baseAttack='Default', attack='Default', bars=None,
...
burning = Effect('Burning', 'Debuff', 'Lose HP every turn', applyRepeated=True, num=20, color=(255, 100, 0), useAtStart=False)
poison = Effect('Poison', 'Debuff', 'Lose HP every turn', applyRepeated=True, num=15, color=(0, 150, 0))
...
#Passive
doubleStrike = Effect('Double Strike', 'Passive', 'Allows you to attack twice', color=(211, 211, 211))
booster = Effect('Boosters', 'Passive', 'Speed increases by 1.75x after Health drops below 50%', color=(150, 255, 150))
glitched = Effect('Glitched', 'Passive', '50% of attack multiplying by 1.25%', color=(150, 255, 150))
lockedIn = Effect('Locked In', 'Passive', 'Attack increase by 30% after Health drops below 30%', color=(255, 255, 0))
iteker = Effect('Iteker', 'Passive', 'After 15 turns, Bleeding will upgrade to a higher stage every 10 turns, starting at Bleeding II.', num=5, color=(150, 0, 0))
regenTurn = Effect('Regeneration', 'Passive', 'Heal every turn', color=(255, 0, 0))
sadism = Effect('Sadism', 'Passive', 'Gain Force I buff for 5 turns when someone dies, whether it’s his enemies or teammates. Stackable', color=(100, 255, 100))
disperse = Effect('Disperse', 'Passive', 'Releases 3 blowballs each in 8 different directions, damaging anyone near.', color=(255, 255, 255))
readyToStrike = Effect('Ready To Strike', 'Passive', 'Attack increases by 1.25x for the rest of the battle if damaged thrice.', color=(210, 50, 50))
productivityZero = Effect('Productivity Zero', 'Passive', 'After being damaged, he becomes weak and his Productivity drops to 0.00x', color=(100, 100, 100))
caneHealing = Effect('Cane Healing', 'Passive', 'After being damaged, all allies within a radius of 1 are healed for 40% of the damage he receives and gain Momentum I for 2 turns. Duration does not stack.', color=(255, 100, 100))
healingTunes = Effect('Healing Tunes', 'Passive', 'Plays on his special keyboard, healing allies within a radius of 1 by 10 HP each cycle.', color=(255, 100, 100))

#bleedSword.applyEffect = {bleeding:[inf, 1]}
#heavySword.applyEffect = {slow:[inf, 1]}

fluffball = Fight('Fluffball', 'Ranged', 'Support', 'Releases extra fluff that has dust and dirt trapped in it. Deals low damage and causes Itch.')

cutStuff = Fight('Just Cut', 'Melee', 'Melee', 'Cuts the chosen target with a knife.', range=1, baseAttack=4, attack=42, barFightAnim={'A1':'attack', 'B1':'attack'})
knifeShot = Fight('Knife Shot', 'Ranged', 'Range', 'Makes a precise shot using the knife.', range=4, baseAttack=3, attack=38, fightAnim='attack_range', barFightAnim={'A1':'attack_range', 'B1':'attack_range'})

tasteThePaper = Fight('Taste the Paper!', 'Thrower', 'Thrower', 'Throw one of the many thick notebooks that are filled with doodles, damaging the target it lands on')

powerBullet = Fight('Power Bullet', 'Ranged', 'Range', 'Fires a green bullet made out of energy and sharp metallic parts, damaging enemies from a far distance.')

spike = Fight('Spike', 'Under Attack', 'Under', 'Create a spike with his powers, damaging the target by making it impale them from the ground.')

strike = Fight('Strike', 'Melee', 'Melee', 'Uses the Sickle to strike a targetted enemy. It have been used before.')

punch = Fight('Punch', 'Melee', 'Tank', 'Ram into the target and unleash a strong punch to beat the hell out of them.')

pencilJab = Fight('Pencil Jab', 'Melee', 'Melee', 'Jab the foes with a strong sharp pencil.')

webBomb = Fight('Web Bomb', 'Thrower', 'Thrower', 'Spins rapidly to create a web bomb to then hurl it at the enemy, casting a Slow I for 3 turns. Adds more when target have the Slow debuff active.', stackDuration=True, applyEffect={slow: [0, 1, 3]})

watchYouBleed = Fight('Watch you bleed', 'Melee', 'Melee', 'Aims for certain part of the opponent, causing them to bleed, casting Bleeding I debuff and last for 1 cycle.', applyEffect={bleeding: [1, 1]})

blowballs = Fight('Blowballs', 'Ranged', 'Range', 'Ejects 5 blowballs that upon contact with the target, deals small amounts of damage each. The further the blowballs travel, the more likely it will miss the target.', attack=13, useHitChance=True, bars=['A1', 'B1', 'C1', 'D1', 'E1'])

blastRay = Fight('Blast Ray', 'Ranged', 'Range', 'A powerful beam, magnified by the Crown power!', bars=['A1', 'A2', 'B1'])

scratch = Fight('Scratch', 'Melee', 'Melee', 'Scratches a target, damaging them.')

crimsonFang = Fight('Crimson Fang', 'Melee', 'Melee', 'Strike a target, damaging them and casting a Confused I debuff on them for 3 turns. Debuff does not stack, resets back to 3 turns if enemy is struck again.', applyEffect={confusion: [3, 1]})
#Under, Slam, Back
winterWhack = Fight('Winter Whack', 'Melee', 'Slam', 'Whacks the target with a candy cane, damaging them.')

weakPoke = Fight('Weak Poke', 'Melee', 'Melee', 'Try to attack the enemy, damaging both the enemy and himself.')

hotTea = Fight('Hot Tea', 'Thrower', 'Thrower', 'The hat throws a hot cup of tea at a target, damaging them and applying Burning I for 2 turns. If hit again while Burning I is active, it resets back to 2. Don’t question how the tea stays in the cup in mid-air.', applyEffect={burning: [0, 1, 2]})

doubleFistBash = Fight('Double Fist Bash', 'Melee', 'Melee', 'Bashes his target with both his fists.', bars=['A1', 'B1'], attack=23, barFightAnim={'A1':'attack', 'B1':'attack_2'}, allAtOnce=False)

lidSlam = Fight('Lid Slam', 'Ram', 'Slam', 'Rush towards the target and slams into them with his lid.')

scream = Fight('SCREAM', 'Spam', 'Tank', 'Floats toward a target and unleashes a bone-chilling scream. The main target takes full damage, and all enemies within a radius of 1 have a 35% chance to be Stunned for 1 turn and take 0.5x the damage dealt to the main target. Stun does not stack')

sharpCane = Fight('Sharp Cane', 'Range', 'Range', 'Fires a sharpened candy cane, damaging the target when hit.')

purplePower = Fight('Purple Power', 'Thrower', 'Thrower', 'Hurls a purple energy ball that bounces off the target, dealing light damage, before hitting the rumbler behind for heavier damage.', attack=18)

sliceTime = Fight('Slice Time', 'Melee', 'Melee', 'Slices a target with a pizza cutter.')

melodyBurst = Fight('Melody Burst', 'Range', 'Range', 'Plays on his special keyboard, sending out damaging musical notes that strike a targeted enemy.')

knockback = Fight('Knockback', 'Melee', 'Melee', 'Deal a powerful punch on a target. Upon a critical hit, has a 50% chance of pushing the target back by 1 space, swapping spaces with the enemy behind the target.')

threeShot = Fight('Three Shot', 'Ranged', 'Range', 'Fires 3 energy shots at her target, damaging them.', attack=14, bars=['A1', 'B1', 'C1'])

_4ero4rm = Fight('4ero 4rm', 'Ranged', 'Range', 'Fires an arm at a target, damaging them from a distance.')

solarDart = Fight('Solar Dart', 'Ranged', 'Hold', 'Charges up a bright energy shot and fires it at her target, inflicting 1 turn of Blindness I.', applyEffect={blindness:[0, 1, 1]})

hornLaunch = Fight('Horn Launch', 'Thrower', 'Thrower', 'Launches both horns, each dealing damage to the target.', attack=21, bars=['A1', 'B1'])
...
check = Act('Check', 'Check an enemy to try to find some useful info about them.', 0, 6, 6, False, 'opponent')
...
fun_doodles = Skill('Fun Doodles', cost=30, type='ally: missingHp', info='Doodles on one of his notebooks, keeping himself more calm and healing himself.',
                    baseNum=70, num=70, usePercent=True,
                    targetType='Self')

blue_slash = Skill('Blue Slash', cost=85, type='opponent: hp',
                   info='Attacks with a fast flying blue slash that pierces through enemies, however, walls are able to block this attack.',
                   baseNum=30, num=30, useAnim=True, animType='Range', targetType='All', applyEffect={confusion:[1, 1]})

fuzzy_encouragement = Skill('Fuzzy Encouragement', cost=50, type='ally: def', info='Give teammates, of radius of 1, some encouragement, giving them Guard I for 6 turns. Guard will increase in stage and reset duration if applied again.',
                            baseNum=0, num=0, targetType='All', applyEffect={guard:[0, 1, 6]}, stackAmplifier=True, maxAmplifier=2)

healing_orb = Skill('Healing Orb', cost=80, type='ally: hp', info='Lobs a healing orb at an ally or himself, granting Healng II buff for one cycle.',
                    baseNum=-10, num=-10, useAnim=True, applyEffect={healing:[1, 2]})

web_wrap = Skill('Web Wrap', cost=75, type='opponent: hp',
                 info='Shoots a web that wraps around his target, trapping them for 2 cycles. If the trapped target is targetted again, 2 more cycles will be added.',
                  animType='Range', stackDuration=True, useAnim=True, applyEffect={trapped:[2, 1]})

copySkill = Skill('Copy', cost=80, type='everyone:', info='Targets someone to copy, taking up their appearance, attacks and abilities.', num=0, baseNum=0)


cane_boost = Skill('Cane Boost', cost=45, type='ally: hp', baseNum=15, num=15, usePercent=False,
                 info='Consumes a Candy Cane, instantly healing 15 HP and granting 5 turns of Momentum II buff to herself. Buff turns can stack in duration',
                  stackDuration=True, applyEffect={momentum:[5, 2]}, targetType='Self')

coffee_charge = Skill('Coffee Charge', cost=55, type='ally: hp', baseNum=-10, num=-10, usePercent=False,
                 info='Give a cup of coffee to an ally, granting Efficiency II for 10 turns and healing 10 HP. Drinking again while still active resets the duration and increases the buff’s stage (Max: Efficiency III).',
                  useAnim=True, stackAmplifier=True, maxAmplifier=3, applyEffect={efficiency:[0, 2, 10]}, targetType='Single')

lid_off = Skill('Lid Off', cost=25, type='ally', baseNum=0, num=0, usePercent=False,
                 info='Takes off his Lid and uses it as a shield, going into Defense mode.',
                  targetType='Self', changePhaseTo='Defense', useDialogue='defense')

lid_on = Skill('Lid On', cost=25, type='ally', baseNum=0, num=0, usePercent=False,
                 info='Puts on his Lid, going into Offense mode.',
...
screamer = Skill('SCREAMER!', cost=90, type='ally: atk', baseNum=1.25, num=1.25, usePercent=False,
                 info='Once activated, he enters his second phase. In this form, his attack is now increased by 1.25x and now has a 45% chance to stun enemies. This ability can only be used once per battle.',
                  targetType='Self', changePhaseTo='Screamer')

hot_and_ready = Skill('Hot and Ready', cost=45, type='ally: hp', baseNum=-30, num=-30, usePercent=False,
                 info="Tosses a fresh slice of pizza to a chosen ally, restoring 30 HP and granting Momentum I buff for 1 turn.",
                 useAnim=True, animType='Thrower', stackAmplifier=True, maxAmplifier=3, applyEffect={momentum:[0, 1, 1]}, useDialogue='skill')

orange_blast = Skill('Orange Blast', cost=82, type='opponent: hp',
                   info='Fires a large orange energy blast, dealing damage and applying 2 turns of Distracted I debuff on the hit target.',
...
he4dshot = Skill('He4dshot', cost=62, type='opponent: hp',
                   info='Shoot his head straight towards a target, dealing heavy damage and stunning them for 1 turn.',
...
spark_charge = Skill('Spark Charge', cost=78, type='ally: def',
                   info='Charges a selected ally, granting them Force II and Shocker I buffs for 5 turns. Buff reset in duration when applied again. For multi-hit rumblers, Shocker I only applies to the first projectile fired per turn.',
...
slash_rage = Skill('Slash Rage', cost=70, type='opponent: hp', info='Lunge multiple slashes at the enemy! Dealing a good chunk of damage!', baseNum=150, repeatHowMany=3, useAnim=True, animType='Melee')

debuff_pierce = Skill('Debuff Pierce', cost=90, type='opponent: hp', info='Unleash a ray that pierces through all enemy! And debuff them with Confusion I for 2 cycles.', baseNum=80, useAnim=True, animType='Range', targetType='All', applyEffect={confusion:[2, 1]})
...
opStat = [5, 5, 5, 5, 5]
lockEn = Opponent('Lock', 'Tank', '', [5, 3, 4, 1, 1, 1], setNum=[165, 30, 33, 8, 1, 1], fight=[punch], action=actions, info='Lock is never wrong. Well, at least that is what he thinks. His failure is not his fault, it is yours.', passiveBuff={lockedIn:[False, False]}, color=(255, 255, 0), skinList=['Default', 'Dark', 'Key Battler', 'Royal Blue'])
...
lockEn = Opponent('Lock', 'Tank', '', [5, 3, 4, 1, 1, 1], setNum=[165, 30, 33, 8, 1, 1], fight=[punch], action=actions, info='Lock is never wrong. Well, at least that is what he thinks. His failure is not his fault, it is yours.', passiveBuff={lockedIn:[False, False]}, barColor=(255, 249, 133), color=(250, 197, 7), skinList=['Default', 'Dark', 'Key Battler', 'Royal Blue'])
...
zeeEn = Opponent('Zee', 'Melee', '', [-3, -2, -2, -3, 0, 0], setNum=[999, 999, 999, 999, 999, 999], maxMana=999, level=99, fight=[zeefight], action=actions, info='DHaiwn8372na0o!28dna82#%^&*(!jhs82728&*@*9d286*(*&^#":Hdjshsjdcx][}{POIhsjwu|[]13==', color=(100, 0, 0), skinList=['Default'])
blue_guyEn = Opponent('Blue Guy', 'Controller', '', [2, 5, 2, 5, 4, 4], setNum=[85, 47, 10, 38, 4, 1.75], fight=[spike], action=actions, canFly=True, skills=[blue_slash], info='Blue Guy is a competitive person who is the brother of Red Dude and cousin of Purple Kid. He was the leader of the Colourful 6, a team that has since disbanded, leaving only the three of them together.', barColor=(133, 134, 255), skinList=['Default', 'Egg Guy'], color=(1, 42, 254))
raysonEn = Opponent('Rayson', 'Range', '', [1, 4, 2, 2, 5, 4], setNum=[70, 44, 18, 20, 5, 1.75], fight=[powerBullet], action=actions, info='Rayson is tired of how robots are being treated and one day plans to take over the world and treat humans the way robots were treated, workers that serve a purpose for the benefits of their creator.', passiveBuff={booster:[False, False], glitched:[False, False]}, barColor=(138, 254, 133), color=(55, 254, 1), skinList=['Default', 'Static', 'Water Gun', 'Summer Blaster'])
calvyEn = Opponent('Calvy', 'Thrower', '', [1, 4, 3, 3, 3, 2], setNum=[56, 45, 29, 28, 3, 1.25], fight=[tasteThePaper], action=actions, skills=[fun_doodles], info='This young boy enjoys drawing a lot. You would expect kids his age to be playing and watching their tablets all day, but Calvy spents hours of his days doodling on the thousands of notebooks in his room.', barColor=(255, 133, 238), color=(255, 1, 254), skinList=['Default', 'Inverted'])
finleyEn = Opponent('Finley', 'All rounder', '', [3, 4, 3, 3, 1, 2], setNum=[115, 42, 12, 30, 1, 1.25], fight=[cutStuff, knifeShot], action=actions, info='Being bullied and laughed at by everyone, Finley is certain that this world is just filled with horrible people. He plans to take down all his enemies one day, one by one, brutally.', passiveBuff={doubleStrike:[False, False]}, barColor=(254, 132, 173), color=(255, 0, 118), skinList=["Default", "Option Two", "Red Vanity", "Blue Vanity"])
...
info='Despite the things Rekety has achieved, he is never satisifed. Maybe one day, he will eventually find something that satisfy him.',
passiveBuff={iteker:[False, False]}, skinList=['Default', 'Rekverty', 'Red', 'Rektmas', 'Candy Cane', 'Rany Swap'], barColor=(101, 103, 255), color=(0, 0, 238))
dandeeEn = Opponent('Dandee', 'Range', '', [1, 5, 2, 2, 5, 4], setNum=[68, 65, 16, 20, 5, 1.75], fight=[blowballs], action=actions, info='Dandee may seem like a harmless dandelion but he is someone you wouldn’t want to mess with. This sadistic dandelion is usually harmless, but when in the mood to kill, his blowballs become dangerous.', passiveBuff={sadism:[False, False], disperse:[False, False]}, barColor=(112, 216, 113), color=(36, 216, 1))
copycatEn = Opponent('Copycat', 'All rounder', '', [3, 2, 2, 3, 2, 4], setNum=[120, 25, 14, 26, 2, 1.75], fight=[scratch], action=actions, skills=[copySkill], info='A fast learner and a master at shapeshifting, many suspect that Copycat is a creature from a planet far far away. Much is unknown about Copycat', barColor=(214, 133, 255), color=(128, 0, 254))
...
skinList=["Default"], barColor=(190, 27, 52), color=(190, 27, 52))
...
skinList=["Default", "Unpixel"], barColor=(190, 27, 52), color=(255, 0, 0))
...
skinList=["Default"], barColor=(190, 27, 52), color=(56, 242, 0))
...
skinList=["Default"], barColor=(190, 27, 52), color=(0, 254, 0), canFly=True, flyType=1, flyRange=10)

mascotEn = Opponent('Mascot', 'Melee', '', [4, 4, 2, 4, 1, 1], setNum=[150, 35, 5, 10, 3, 1], fight=[strike], action=actions, info='Someone that is meant for a fighting dummy.', color=(255, 255, 255), skinList=['Default'])
...
blue_guy = Player('Blue Guy', 'Controller', '', [2, 5, 2, 5, 4, 4], setNum=[85, 47, 10, 38, 4, 1.75], fight=[spike], action=actions, canFly=True, skills=[blue_slash], info='Blue Guy is a competitive person who is the brother of Red Dude and cousin of Purple Kid. He was the leader of the Colourful 6, a team that has since disbanded, leaving only the three of them together.', barColor=(133, 134, 255), skinList=['Default', 'Egg Guy'], color=(1, 42, 254))
rayson = Player('Rayson', 'Range', '', [1, 4, 2, 2, 5, 4], setNum=[70, 44, 18, 20, 5, 1.75], fight=[powerBullet], action=actions, info='Rayson is tired of how robots are being treated and one day plans to take over the world and treat humans the way robots were treated, workers that serve a purpose for the benefits of their creator.', passiveBuff={booster:[False, False], glitched:[False, False]}, barColor=(138, 254, 133), color=(55, 254, 1), skinList=['Default', 'Static', 'Water Gun', 'Summer Blaster'])
calvy = Player('Calvy', 'Thrower', '', [1, 4, 3, 3, 3, 2], setNum=[56, 45, 29, 28, 3, 1.25], fight=[tasteThePaper], action=actions, skills=[fun_doodles], info='This young boy enjoys drawing a lot. You would expect kids his age to be playing and watching their tablets all day, but Calvy spents hours of his days doodling on the thousands of notebooks in his room.', barColor=(255, 133, 238), color=(255, 1, 254), skinList=['Default', 'Inverted'])
finley = Player('Finley', 'All rounder', '', [3, 4, 3, 3, 1, 2], setNum=[115, 42, 12, 30, 1, 1.25], fight=[cutStuff, knifeShot], action=actions,
info='Being bullied and laughed at by everyone, Finley is certain that this world is just filled with horrible people. He plans to take down all his enemies one day, one by one, brutally.',
...
info='Despite the things Rekety has achieved, he is never satisifed. Maybe one day, he will eventually find something that satisfy him.',
passiveBuff={iteker:[False, False]}, skinList=['Default', 'Rekverty', 'Red', 'Rektmas', 'Candy Cane', 'Rany Swap'], barColor=(101, 103, 255), color=(0, 0, 238))
dandee = Player('Dandee', 'Range', '', [1, 5, 2, 2, 5, 4], setNum=[68, 65, 16, 20, 5, 1.75], fight=[blowballs], action=actions, info='Dandee may seem like a harmless dandelion but he is someone you wouldn’t want to mess with. This sadistic dandelion is usually harmless, but when in the mood to kill, his blowballs become dangerous.', passiveBuff={sadism:[False, False], disperse:[False, False]}, barColor=(112, 216, 113), color=(36, 216, 1))
copycat = Player('Copycat', 'All rounder', '', [3, 2, 2, 3, 2, 4], setNum=[120, 25, 14, 26, 2, 1.75], fight=[scratch], action=actions, skills=[copySkill], info='A fast learner and a master at shapeshifting, many suspect that Copycat is a creature from a planet far far away. Much is unknown about Copycat', barColor=(214, 133, 255), color=(128, 0, 254))
...
validAlly = [lock, li_wei, blue_guy, rayson, calvy, finley, taphygrafy, rany, ocho, rekety, dandee, copycat, red_dude, candace, thin, mr_grafy, jam_jar, screamy, caen, purple_kid, locon, t_music, blue_black, orange_girl, fourminti, yellow_spark, gregley]
