...
class Sprite:
    ...
    def draw(self, x=0, y=0):
        image = loadImg((self.image, self.baseScale * self.scale, self.color + (self.opacity,)), self.flipX, self.angle, doCrop=True, blendMode='MULT')
        if useTexture:
...
class AnimatedSprite(Sprite):
    ...
    def draw(self, x=0, y=0):
        image = loadImg((self.getCurrentFrame(), self.baseScale * self.scale, self.color + (self.opacity,)), self.flipX, self.angle, blendMode='MULT')
        doColorTint = False
        if self.tweenActiveColor:
            doColorTint = True
            tintOpacity = 255*(1 - (self.timeColor/self.durationColor))
            tintColor = loadImg((self.getCurrentFrame(), self.baseScale * self.scale, self.tintColor + (tintOpacity,)), self.flipX, self.angle, blendMode='ADD')
...
def loadSprite(spriteName, image='meter.png', pos=(0,0), baseScale=1, scale=1, flipX=False, angle=0, color=(255, 255, 255), opacity=255, replace=False,
               mouseCollide=False, pivotType='center', layer=screen):
    if spriteName not in sprites.keys() or replace:
        sprites[spriteName] = Sprite(spriteName, image, pos, baseScale, scale, flipX, angle, color, opacity, mouseCollide=mouseCollide, pivotType=pivotType, layer=layer)
...
class Button(Sprite):
    ...
    def draw(self):
        ...
        buttonText = render_text(self.name, self.textColor, self.textSize, True, BLACK, 3)
