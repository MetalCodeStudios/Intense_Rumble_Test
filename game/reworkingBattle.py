...
def battleText(usePotrait=False, name=None, forceDo=False, emotion=None):
    ...
    for num, line in enumerate(lines):
        ...
        blitObj(hud, lineText, hudRect[4] + 20 + xOffset, y, pivot_type="topleft")
...
def battleAlly():
    ...
    for i, ally in enumerate(party):
        ...
        y = (height - 95) - allyTween[i] + shakingY + battleStart
        if len(party) > 5 and ally == party[selectedAlly]:
            x = 500
        ...
        if ally.tempClass in ['Tank', 'Controller']:
            ...
            if ally.tempClass == 'Controller':
                for i in range(0, random.randint(4, 6)):
                    if ally.tempClass == 'Controller':
                        keys.append(random.choice(string.ascii_lowercase))#['z', 'x', 'c']))
                        keyUsedBool.append(False)
                keyUsed.append(keys)
        ...
        if item["Bool"] == True:
            ...
            for targetNum, target in enumerate(item["Targets"]):
                ...
                if targetSprite.checkCollision(projectileSprite) and target.hp > 0 and item["TargetTracker"][targetNum] == False:
                    ...
                    fightyLoop.append([target, item["Attacker"], item["Damage"]])
                    ...
                    if not firstPerson:
                        ...
                        if not i[7][num]:
                            if i[2] == 'Thrower':
                                ...
                                #projSprite.flipX = attacker.flipX
                            elif i[2] == 'Controller' or i[2] == 'Under':
...
def main(Players, Enemies, useSpeedTurn=True, usingDebug=False, battleData=None):
    ...
    global onStart, onUpdate, actCall, frame, onEnd, useMobile, allowGameOver, allowWin
    def onUpdate():
        pass
    def onStart():
        pass
    ...
    if bossFight:
        ...
        for i in party:
            ...
            #pygame.mixer.music.load(f'data/sounds/music/ZephyrMusic.mp3')
            #playSound('music/ZephyrMusic.ogg', True)
            useExtraDialogue = False
        #party[0].skills[0].cost = 30
        party[0].skills.append(character.life_bless)
        party[1].skills.append(character.focus)
        party[1].skills.append(character.slash_rage)
        useDodgeEquation = True
        turnDialogue = [[[None, None, None], ['Zephyr ambushed the party!'], False, 0, [None]]]
        addTurnDialogue('Rekety', "What the hell is that?!?", 2)
        addTurnDialogue('Rekety', "Where even are we?!", 2)
        addTurnDialogue('Rany', "...", 2)
        addTurnDialogue('Rekety', "Rany, do you know what is that entity??", 3)
        addTurnDialogue('Rany', "It's...", 3)
        addTurnDialogue('Rany', "One of their victims...", 3)
        addTurnDialogue('Zephyr', "Submit, or face annihilation.", 4, 'Smug')
        addTurnDialogue('Rekety', "How do we even beat it?", 5)
        addTurnDialogue('Rany', "We need to attack him repeatedly.", 5)
        addTurnDialogue('Rekety', "Couldn't we try to reason with him?", 6)
        addTurnDialogue('Rany', "It's Possible, but it may be tedious.", 6)
        addTurnDialogue('Rany', "Plus, it's difficult for such wearer to want to willingly give up.", 6)
        addTurnDialogue('Zephyr', "This is my last offer.", 7, 'Neutral')
        addTurnDialogue('Zephyr', "Join me and be granted everything you desire", 7, 'Smug')
        addTurnDialogue('Zephyr', "Or face total destruction!", 7, 'Angry')
        addTurnDialogue('Rany', "Hell no!!!", 7)
        addTurnDialogue('Rekety', "I wish to not join too.", 7)
        addTurnDialogue('Zephyr', "Then prepare to suffer!", 7, 'Angry')
        addTurnDialogue('Rekety', "We should try talking regardless. I mean if you're not fine with it then I guess we can continue attacking.", 8)
        addTurnDialogue('Rekety', "And if you're not fine with it then I guess we can continue attacking.", 8)
        addTurnDialogue('Rany', "...", 8)
...
async def battleRun():
    ...
    while inBattle:
        ...
        if buff_tweenY < 700:
            ...
            iconImg = loadImg((f'selectionIcon/{ally.name}.png', 0.75))
       ...
       # backGroundScreen.fill((120, 120, 180, 180))
        ...
        if botplay:
            ...
            blitObj(screen, botPlayText, width/2, 50)
            ...
            if inSkill:
                if text_index >= len(splashy) and actionWhich[skillParty] == 'usedSkills':
                    ...
                    if skillParty < len(party) - 1:
                        while 'Skills' not in actionWhich[skillParty] and skillParty < len(theAlly) - 1:
                            skillParty += 1
                ...
                if text_index >= len(splashy) and actionNum == len(actionDia) - 1 and backUpCoords == []:
                    ...
                    if itemParty < len(theAlly) - 1:
                        while 'Act' not in actionWhich[actParty] and actParty < len(theAlly) - 1:
                            itemParty += 1
                ...
                if text_index >= len(splashy):
                    if text_index >= len(splashy) and actionWhich[itemParty] == 'usedItems':
                        ...
                        addInBar()
                        if itemParty < len(theAlly) - 1:
                            while 'Items' not in actionWhich[itemParty] and itemParty < len(theAlly) - 1:
                                itemParty += 1
            ...
            if ('Skills' in actionWhich or 'usedSkills' in actionWhich or 'doneSkills' in actionWhich):
                        ...
                        if attackFrame[num] == 0 and animate:
                            if actionWhich[skillParty] == 'Skills':
                                ...
                                if isinstance(whichSelection[num], list):
                                    ...
                                    attacking.append([ allOpponents , theAlly[num], skills.animType, skills.num, False, skills, None])
                                else:
                                    for repeatation in range(0, skills.repeatHowMany):
                                        if 'opponent' in skills.type:
                                            attacking.append([ [theOpponent[whichSelection[num]]] , theAlly[num], skills.animType, skills.num, False, skills, None])
                                        else:
                ...
                if meleeUsed and not meleeDone:
                    ...
                    if int(hudRect[2]) == 527 and controllerFound:
                        ...
                        for number, i in enumerate(theAlly):
                            if i.tempClass != 'Controller':
                                continue
                            ...
                            if opponentBot and turnEnemy:
                                ...
                                speedDifference = abs(theOpponent[whichSelection[number]].speed - theAlly[number].speed) / 100
                                speedModifier = 1 + speedDifference if theOpponent[whichSelection[number]].speed > theAlly[number].speed else 1 - abs(speedDifference)
                                base = 0
                                further = 0
                                baseNum = 1*theDelta
                            ...
                            if keyDone[number] == False:
                                fightBarX[number] += baseNum*speedModifier
                            if fightBarX[number] >= 70 or keyHit[number] == len(keyUsed[number]):
                                keyDone[number] = True
                                multiplier = abs(keyHit[number]/len(keyUsed[number]) - base) + further
                                added = 0 if multiplier < 1 else 0.3
                                multiplier += added
                                if useDefensePercent:
                                    damage = i.attack * ((100 - theOpponent[whichSelection[number]].defense)/100)
                                else:
                                    damage = i.attack - theOpponent[whichSelection[allyNum]].defense
                            ...
                            hit = render_text(f'Hits: {keyHit[number]}/{len(keyUsed[number])}', WHITE, 40)
                            blitObj(hud, hit, hpBG_X + 240, hpBG_Y, 'topleft')
                            numUsed = 0

                            for num, i in enumerate(keyUsed[number]):
                                variable = {letter: None for letter in string.ascii_lowercase}
                                x = hudRect[4] + 80*num + 30
                                y = hudRect[5] + 30
                                key_pressed = pygame.key.get_pressed()[getattr(pygame, f"K_{i}")]

                                if (key_pressed or keyUsedBool[num] == True) and num == keyHit[number] and numUsed == 0 and keyDone[number] == False:
                                    numUsed = 1
                                    keyHit[number] += 1

                                if num < keyHit[number]:
                                    buttonColor = YELLOW
                                    scale = 70
                                else:
                                    buttonColor = WHITE
                                ...
                                letter= render_text(f'{i.upper()}', GRAY, 60*scale/70)
                                blitObj(hud, letter, x + 35, y + 35)
        ...
        if whichTurn:
            ...
            if turnAlly:
                ...
                for num, ally in enumerate(theOpponent[::-1]):
...
#main([character.rayson, character.rany, character.li_wei], [character.allerwaveEn, character.ranyEn, character.reketyEn], usingDebug=True)
